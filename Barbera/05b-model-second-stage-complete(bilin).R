...
lpd <- function(alpha, beta, theta, phi, mu_beta, sigma_beta, y){
    require(arm, quiet=TRUE)
    value <- alpha + beta + (theta * phi)
...
metropolis.logit <- function(y, alpha.i, phi.i, mu_beta.i, sigma_beta.i, beta.init, theta.init, 
    iters=2000, delta=0.05, chains=2, n.warmup=1000, thin=1, verbose=FALSE)
{
    ...
    for (chain in 1:chains){
        ...
        accept.prob <- exp(lpd(alpha, beta=pars.cand[1], theta=pars.cand[2], phi, mu_beta, sigma_beta, y) - 
            lpd(alpha, beta=pars.cur[1], theta=pars.cur[2], phi, mu_beta, sigma_beta, y))
...
estimation <- function(first, last=first+4999){
    ...
    for (i in 1:length(pars)){
        fit <- metropolis.logit(y[pars[i],], alpha.i, phi.i, mu_beta.i, sigma_beta.i,
...
load(matrixfile)
y <- y[n1:(n1+49999),]
results1 <- mclapply(seq(n1,n2,5000), estimation, mc.cores=6)
